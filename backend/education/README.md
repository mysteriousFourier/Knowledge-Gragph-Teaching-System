...
在单端口 FastAPI 模式下，本模块不会单独暴露 `8001`，而是由根目录 `render_app.py` 把 `/api/education/...` 和 `/api/student/...` 路由挂到同一个 Web Service 中。
...
单个选项的点赞/点踩只作用于该选项。坏选项确认后会尝试只重写该选项，避免其它已满意的题干、正确答案或选项被重新约束。
