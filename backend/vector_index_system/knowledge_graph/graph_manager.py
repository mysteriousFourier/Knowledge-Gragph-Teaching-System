...
class KnowledgeGraphManager:
    ...
    def _init_database(self):
        ...
        try:
            cursor.execute("ALTER TABLE nodes ADD COLUMN metadata_json TEXT")
    ...
    def _create_node_safe(self, id, label, type, content, source, chapter=None, extra_metadata=None):
        """安全的节点创建（带ID检查），支持自定义ID和metadata"""
        existing = self.get_node_by_custom_id(id)
        if existing:
            if not existing.get('chapter') and chapter:
                self._update_node_metadata(id, {'chapter': chapter})
            return existing['id']
        ...
        try:
            cursor.execute("""
                INSERT INTO nodes (id, label, type, content, metadata_json, source, confidence, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (id, label, type, content, metadata_json, source, 1.0, timestamp, timestamp))
            conn.commit()
            return id
        except Exception as e:
            print(f"创建节点失败: {e}")
    ...
    def _update_node_metadata(self, node_id, metadata_update):
        """更新节点的metadata_json字段（合并更新）"""
        conn = sqlite3.connect(str(self.db_path))
        cursor = conn.cursor()

        try:
            cursor.execute("SELECT metadata_json FROM nodes WHERE id = ?", (node_id,))
            row = cursor.fetchone()
            if row:
                existing = {}
                if row[0]:
                ...
                """, (json.dumps(existing, ensure_ascii=False), timestamp, node_id))
                conn.commit()
        except Exception as e:
            print(f"更新节点metadata失败: {e}")
            conn.rollback()
        finally:
            conn.close()
    ...
    def get_node_by_custom_id(self, custom_id):
        ...
        try:
            cursor.execute("SELECT id, label, type, content, metadata_json FROM nodes WHERE id = ?", (custom_id,))
            row = cursor.fetchone()
            if row:
                metadata = {}
                if row[4]:
...
<graphml xmlns="http://graphml.graphdrawing.org/xmlns">
  ...
  <key id="chapter" for="node" attr.name="chapter" attr.type="string"/>
