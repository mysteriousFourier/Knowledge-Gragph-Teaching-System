...
def _markdown_heading_id(title: str, index: int) -> str:
    slug = re.sub(r"[^\w\u4e00-\u9fff]+", "_", title, flags=re.UNICODE).strip("_").lower()
    return f"md::{slug or index}"


def import_markdown_graph_payload(markdown: str, *, filename: str = "") -> Dict[str, Any]:
    """Import a readable Markdown knowledge-graph file as chapter/concept nodes.

    Headings become nodes. Markdown links like [target](#target) create related
    edges when the target heading exists. This keeps the source text readable in
    the UI while still making the graph usable by retrieval.
    """
    ...
    def flush_current() -> None:
        ...
        current["content"] = body or current["metadata"]["label"]
    ...
    for line in lines:
        ...
        if match:
            ...
            node_id = _markdown_heading_id(title, len(nodes) + 1)
            heading_ids[title.lower()] = node_id
            heading_ids[re.sub(r"[^\w\u4e00-\u9fff]+", "-", title, flags=re.UNICODE).strip("-").lower()] = node_id
            current = {
                "id": node_id,
                "content": "",
                "type": "chapter" if level <= 2 else "concept",
                "metadata": {
                    "id": node_id,
                    "label": title,
                    "level": level,
                    "source": "markdown_graph",
    ...
    if not nodes and markdown.strip():
        title = Path(filename).stem or "Markdown knowledge graph"
        node_id = _markdown_heading_id(title, 1)
        nodes.append({
            "id": node_id,
    ...
    for index, node in enumerate(nodes):
        if index > 0:
            relations.append({
                ...
                "target_id": node["id"],
                "relation_type": "next",
                "metadata": {"source": "markdown_graph", "source_file": filename},
            })

        text = f"{node.get('content') or ''}\n{(node.get('metadata') or {}).get('label') or ''}"
        for _label, raw_target in re.findall(r"\[([^\]]+)\]\(([^)]+)\)", text):
            target_key = raw_target.strip().lstrip("#").replace("%20", " ").lower()
            target_key = re.sub(r"[^\w\u4e00-\u9fff]+", "-", target_key, flags=re.UNICODE).strip("-")
            target_id = heading_ids.get(target_key)
    ...
    result = call_backend_tool("batch_import_graph", {"nodes": nodes, "relations": relations})
    result["markdown_stats"] = {
        "nodes_parsed": len(nodes),
        "relations_parsed": len(relations),
    }
