  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">管理章节、生成授课文案、PPT 和练习题</p>
      </div>
      ...
      <div className="route-grid">
        <QuickActionCard
          ...
          title="备课模式"
          description="导入 Markdown 知识图谱并生成文案"
        />
        <QuickActionCard
          to="/teacher/ppt"
          icon={<FileUp className="h-5 w-5" />}
          title="PPT 文稿"
          description="从 Markdown 知识图谱生成 PPT"
        />
        <QuickActionCard
          to="/teacher/lecture"
          icon={<FileText className="h-5 w-5" />}
          title="授课模式"
          description="播放和编辑讲解内容"
        />
        <QuickActionCard
      ...
      <div>
        <h2 className="mb-4 text-lg font-semibold">已保存章节</h2>
        {isLoading ? (
          <LoadingSpinner text="加载章节中..." />
        ) : chapters.length === 0 ? (
          <EmptyState title="暂无章节" description="当前没有已保存的章节，请在备课模式导入 Markdown 知识图谱文件。" />
        ) : (
...
function QuickActionCard({
  ...
  return (
    <Link to={to} className="flex min-h-24 items-center gap-4 bg-card p-4 transition-all">
    ...
    <div className="group flex flex-col border bg-card p-5 transition-all">
      ...
      <div className="mt-auto flex items-center gap-2">
        ...
        <Link
          to="/teacher/lecture"
          search={{ chapterId: chapter.id }}
          className="flex-1 rounded-lg bg-secondary py-2 text-center text-sm font-medium text-secondary-foreground transition-colors hover:bg-secondary/80"
        >
          授课
        </Link>
        <button
