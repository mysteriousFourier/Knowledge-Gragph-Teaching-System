...
export const useGenerateBeamerLatex = () => {
  return useMutation({
    mutationFn: (data: { content: string; style?: string; slide_count?: number }) =>
      educationClient
        .post<BeamerGenerateResponse>("/api/education/beamer/generate", data, {
          timeout: 0,
        })
        .then((r) => r.data),
  })
...
export const useParseBeamerLatex = () => {
  return useMutation({
    mutationFn: (latex: string) =>
      educationClient
        .post<BeamerParseResponse>("/api/education/beamer/parse", { latex })
        .then((r) => r.data),
  })
}

export const exportBeamerPptx = async (slidesData: BeamerSlidesData) => {
  const response = await educationClient.post<Blob>(
    "/api/education/beamer/export-pptx",
