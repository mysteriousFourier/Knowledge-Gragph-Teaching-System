...
Now I've heard a Bayesian is half a man With robustness not on his side, But as the problems get tougher, I just watch and wonder As the frequentists run and hide!
...
## appendix2_002 · Appendix: Introduction / WHY ARE BAYESIAN METHODS BECOMING MORE POPULAR?
...
## appendix2_003 · Appendix: Introduction / BAYES' THEOREM

**[命题 Proposition]**

The foundation of Bayesian statistics is Bayes' theorem. Suppose we observe a random variable, x, and wish to make inferences about another random variable, $ \theta $, in the case where $ \theta $ is drawn from some distribution, $ \operatorname{Pr}(\theta) $. From the definition of conditional probability $$ \mathrm{Pr}(\theta\mid x)=\frac{\mathrm{Pr}(x,\theta)}{\mathrm{Pr}(x)} $$ where (for now) x and $ \theta $ are discrete random variables. Again from the definition of conditional probability, we can express the joint probability by conditioning on $ \theta $ to give $$ \Pr(x,\theta)=\Pr(x\mid\theta)\Pr(\theta) $$

**[命题 Proposition]**

Putting these together yields Bayes' theorem $$ \mathrm{Pr}(\theta\mid x)=\frac{\mathrm{Pr}(x\mid\theta)\mathrm{Pr}(\theta)}{\mathrm{Pr}(x)} $$

**[命题 Proposition]**

Bayes' theorem flips the conditioning variable, allowing us to move from $ \Pr(x \mid \theta) $ to $ \Pr(\theta \mid x) $. With $ k $ possible values of $ \theta $ ($ \theta_1 $ through $ \theta_k $), the discrete version of Bayes' theorem becomes $$ \Pr(\theta_{j}\mid x)=\frac{\Pr(x\mid\theta_{j})\Pr(\theta_{j})}{\Pr(x)}=\frac{\Pr(x\mid\theta_{j})\Pr(\theta_{j})}{\displaystyle\sum_{i=1}^{k}\Pr(\theta_{i})\Pr(x\mid\theta_{i})}\quad for\quad1\leq j\leq k $$
...
All of the above statements also hold for continuous random variables, for which the probability density function, p, replaces the discrete probability value, Pr. In particular, the continuous multivariate version of Bayes' theorem is $$ p(\boldsymbol{\Theta}\mid\mathbf{x})=\frac{p(\mathbf{x}\mid\boldsymbol{\Theta})p(\boldsymbol{\Theta})}{p(\mathbf{x})}=\frac{p(\mathbf{x}\mid\boldsymbol{\Theta})p(\boldsymbol{\Theta})}{\int p(\mathbf{x},\boldsymbol{\Theta})d\boldsymbol{\Theta}} $$ where $ \boldsymbol{\Theta} = (\theta_1, \theta_2, \cdots, \theta_n) $ is a vector of $ n $ (potentially) continuous variables. As with the univariate case, $ p(\boldsymbol{\Theta}) $ is the assumed prior distribution of the unknown parameters, while $ p(\boldsymbol{\Theta} \mid \mathbf{x}) $ is the posterior distribution given the prior, $ p(\boldsymbol{\Theta}) $, and the data, $ \mathbf{x} $.
...
>
> To solve this problem using Bayes' theorem, we first define the indicator random variable $$ \theta=\left\{\begin{aligned}0&\quad son is Bb\\ 1&\quad son is BB\end{aligned}\right. $$
>
> Given that both parents are Bb, the expected priors for their offspring are 1/4 for BB and 1/2 for Bb, resulting in a 3/4 prior for a black-coated offspring. Further, from conditional probability (Equation A2.1a), the prior that a black offspring is BB is $$ \Pr(BB\mid\mathrm{Black})=\frac{\Pr(BB,\mathrm{Black})}{\Pr(\mathrm{Black})}=\frac{\Pr(BB)}{\Pr(\mathrm{Black})}=\frac{1/4}{3/4}=1/3 $$ where we used the fact that all BB are black, so that $ \Pr(BB, \text{Black}) = \Pr(BB) $. Hence, the prior becomes $$ \Pr(\theta)=\left\{\begin{aligned}&0&is&2/3\\ &1&is&1/3\end{aligned}\right. $$
>
> Further $$ \Pr(all n offspring are black\mid sire is BB)=1 $$ $$ \Pr(all n offspring are black\mid sire is Bb)=(1/2)^{n} $$ and $$ \begin{aligned}Pr(all n black)&=Pr(all black\mid BB)^{*}Pr(BB)+Pr(all black\mid Bb)^{*}Pr(Bb)\\&=1\cdot1/3+(1/2)^{n}\cdot(2/3)\end{aligned} $$
>
> If we combine the above values, Bayes' theorem yields $$ \Pr(\theta=1\mid n black offspring)=\frac{\Pr(n\mid\theta=1)Pr(\theta=1)}{\Pr(n)}=\frac{1\cdot(1/3)}{1\cdot(1/3)+(1/2)^{n}\cdot(2/3)} $$ which returns values of 0.5, 0.67, 0.8, 0.89, 0.94, and 0.998 for n = 1, 2, 3, 4, 5, and 10, respectively.
...
## appendix2_004 · Appendix: Introduction / FROM LIKELIHOOD TO BAYESIAN ANALYSIS
...
To transition from a likelihood to a Bayesian analysis, we start with some prior distribution, $ p(\boldsymbol{\Theta}) $, that captures our initial knowledge (or best guess) about the possible values of the unknown parameters. From Bayes' theorem, the data (likelihood) is combined with the prior to produce a posterior distribution, $$ p(\boldsymbol{\Theta}\mid\mathbf{x})=\frac{1}{p(\mathbf{x})}\cdot p(\mathbf{x}\mid\boldsymbol{\Theta})\cdot p(\boldsymbol{\Theta}) $$ $$ =(normalizing constant)\cdot p(\mathbf{x}\mid\boldsymbol{\Theta})\cdot p(\boldsymbol{\Theta}) $$ $$ =constant\cdot likelihood\cdot prior $$ as $ p(\mathbf{x}|\boldsymbol{\Theta}) = \ell(\boldsymbol{\Theta}|\mathbf{x}) $ is simply the likelihood function (LW Appendix 4) and $ 1/p(\mathbf{x}) $ is a constant (with respect to $ \boldsymbol{\Theta} $). Consequently, the posterior distribution is often written as $$ p(\boldsymbol{\Theta}\mid\mathbf{x})\propto\ell(\boldsymbol{\Theta}\mid\mathbf{x})~p(\boldsymbol{\Theta}) $$ where the symbol $ \propto $ means “proportional to” (equal up to a constant). Note that the constant $ p(\mathbf{x}) $ normalizes $ p(\mathbf{x} \mid \boldsymbol{\Theta}) \cdot p(\boldsymbol{\Theta}) $ to one, and hence can be obtained by integration $$ p(\mathbf{x})=\int_{\boldsymbol{\varTheta}}p(\mathbf{x}\mid\boldsymbol{\varTheta})\cdot p(\boldsymbol{\varTheta})d\boldsymbol{\varTheta} $$

The dependence of the posterior on the prior (which can easily be assessed by trying different priors) provides an indication of how much information on the unknown parameter values is contained in the data (the curvature of the likelihood surface). If the posterior is highly dependent on the prior, then the data likely has little signal (a flat likelihood surface), while if the posterior is largely unaffected by different priors, then the data are likely highly informative (a sharply peaked likelihood surface). To see this, taking logs on Equation A2.4c yields $$ \log(posterior)=\log(likelihood)+\log(prior)+constant $$
...
## appendix2_006 · Appendix: Introduction / SUMMARIZING THE POSTERIOR DISTRIBUTION
...
We could take the expected value of $ \theta $ (its mean) given the posterior $$ \widehat{\boldsymbol{\theta}}=\boldsymbol{E}[\boldsymbol{\theta}\mid\mathbf{x}]=\int\boldsymbol{\theta}\boldsymbol{p}(\boldsymbol{\theta}\mid\mathbf{x})d\boldsymbol{\theta} $$

Another candidate is the median of the posterior, which is more robust than the mean to outliers. Here the estimator satisfies $ \Pr(\theta > \widehat{\theta} | \mathbf{x}) = \Pr(\theta < \widehat{\theta} | \mathbf{x}) = 0.5 $, hence $$ \int_{\widehat{\theta}}^{+\infty}p(\theta\mid\mathbf{x})d\theta=\int_{-\infty}^{\widehat{\theta}}p(\theta\mid\mathbf{x})d\theta=\frac{1}{2} $$
...
In the classical statistical framework of the frequentists, one uses the observed data to test the significance of a particular hypothesis, and (if possible) compute a p value (the probability, p, of observing a value equal to, or more extreme than, that of the test statistic if the null hypothesis is indeed correct). Initially, one would think that the idea of a hypothesis test is trivial in a Bayesian framework, as using the posterior distribution provides the expected p values directly, for example, $$ \Pr(\theta>\theta_{0})=\int_{\theta_{0}}^{\infty}p(\theta\mid\mathbf{x})d\theta\qquad and\qquad\Pr(\theta_{0}<\theta<\theta_{1})=\int_{\theta_{0}}^{\theta_{1}}p(\theta\mid\mathbf{x})d\theta $$

The fault in this logic under a Bayesian framework is that we also have prior information and Bayesian hypothesis testing addresses whether, given the data, we are more or less inclined to believe the hypothesis than was suggested from the prior. Hence, the prior probabilities influence hypothesis testing. To formalize this idea, let $$ p_{0}=\Pr(\theta\in\Theta_{0}\mid\mathbf{x})\qquad and\qquad p_{1}=\Pr(\theta\in\Theta_{1}\mid\mathbf{x}) $$ denote the probabilities, given the observed data, $ \mathbf{x} $, that $ \theta $ is in the null $ (p_0) $ and alternative $ (p_1) $ hypothesis sets. Note that these are posterior probabilities. Because $ \Theta_0 \cap \Theta_1 = \emptyset $ and $ \Theta_0 \cup \Theta_1 = \Theta $, it follows that $ p_0 + p_1 = 1 $. Likewise, for the prior probabilities we have $$ \pi_{0}=\Pr(\theta\in\Theta_{0})\qquad and\qquad\pi_{1}=\Pr(\theta\in\Theta_{1}) $$ Thus the prior odds of H0 versus H1 are π0/π1, while the posterior odds are p0/p1.

The Bayes factor, $ B_{0} $, in favor of $ H_{0} $ versus $ H_{1} $ is calculated by the ratio of the posterior odds divided by the prior odds, $$ B_{0}=\frac{p_{0}/p_{1}}{\pi_{0}/\pi_{1}}=\frac{p_{0}\pi_{1}}{p_{1}\pi_{0}} $$

The Bayes factor is loosely interpreted as the odds in favor of $ H_0 $ over $ H_1 $ as given by the data and our prior opinion. Because $ \pi_1 = 1 - \pi_0 $ and $ p_1 = 1 - p_0 $, we can also express this as $$ B_{0}=\frac{p_{0}(1-\pi_{0})}{\pi_{0}(1-p_{0})} $$
...
## appendix2_009 · Appendix: Introduction / THE CHOICE OF A PRIOR
...
The Jeffreys prior is as follows: $$ p(\theta)\propto\sqrt{I(\theta\mid\mathbf{x})} $$

A full discussion, with derivation, can be found in Lee (2012).

When there are $ k $ parameters, I is the $ k \times k $ Fisher information matrix of the expected second partials, where the elements of I are calculated by $$ \mathbf{I}(\boldsymbol{\Theta}|\mathbf{x})_{i j}=-E_{x}\left[\frac{\partial^{2}\ln\ell(\boldsymbol{\Theta}|\mathbf{x})}{\partial\theta_{i}\partial\theta_{j}}\right] $$

In this case, the Jeffreys prior becomes $$ p(\boldsymbol{\Theta})\propto\sqrt{\det[\mathbf{I}(\boldsymbol{\theta}\mid\mathbf{x})]} $$
...
>
> Thus $$ \frac{\partial L(\theta\mid\mathbf{x})}{\partial\theta}=\frac{x}{\theta}-\frac{n-x}{1-\theta} $$ and likewise $$ \frac{\partial^{2}L(\theta\mid\mathbf{x})}{\partial\theta^{2}}=-\frac{x}{\theta^{2}}-(-1)\cdot(-1)\frac{n-x}{(1-\theta)^{2}}=-\left(\frac{x}{\theta^{2}}+\frac{n-x}{(1-\theta)^{2}}\right) $$ Because $ E[x] = n\theta $, then $$ -E\left[\frac{\partial^{2}\ln\ell(\theta\mid\mathbf{x})}{\partial\theta^{2}}\right]=\frac{n\theta}{\theta^{2}}+\frac{n(1-\theta)}{(1-\theta)^{2}}=n\theta^{-1}(1-\theta)^{-1} $$
>
> The resulting Jeffreys prior for this likelihood becomes $$ p(\theta)\propto\sqrt{\theta^{-1}(1-\theta)^{-1}}\propto\theta^{-1/2}(1-\theta)^{-1/2} $$ which is a U-shaped beta distribution with parameters $ \alpha = \beta = 1/2 $ (Equation A2.38a). This prior puts more weight on extreme values relative to assuming a uniform over $ (0,1) $, see Figure A2.3.
...
## appendix2_012 · Appendix: Introduction / POSTERIOR DISTRIBUTIONS UNDER NORMALITY ASSUMPTIONS

**[命题 Proposition]**

To introduce the basic ideas of Bayesian analysis, as well as treating a common assumption in quantitative genetics, consider the case where data are drawn from a normal (Gaussian) distribution, giving the likelihood function for the ith observation, $ x_{i} $, as $$ \ell(\mu,\sigma^{2}\mid x_{i})=\frac{1}{\sqrt{2\pi\sigma^{2}}}\exp\left(-\frac{(x_{i}-\mu)^{2}}{2\sigma^{2}}\right) $$

If we assume independence, the resulting full likelihood for all n data points (with a sample mean of $ \overline{x} $) is $$ \ell(\mu\mid\mathbf{x})=\frac{1}{\sqrt{2\pi\sigma^{2}}}\exp\left(-\sum_{i=1}^{n}\frac{(x_{i}-\mu)^{2}}{2\sigma^{2}}\right) $$ $$ =\frac{1}{\sqrt{2\pi\sigma^{2}}}\exp\left[-\frac{1}{2\sigma^{2}}\left(\sum_{i=1}^{n}x_{i}^{2}-2\mu n\overline{x}+n\mu^{2}\right)\right] $$
...
A useful device when calculating the posterior distribution is to ignore terms that are constants with respect to the unknown parameters. Suppose x denotes the data and $ \Theta_1 $ is a vector of known model parameters, while $ \Theta_2 $ is a vector of unknown parameters. If we can write the posterior as $$ p(\boldsymbol{\Theta}_{2}\mid\mathbf{x},\boldsymbol{\Theta}_{1})=f(\mathbf{x},\boldsymbol{\Theta}_{1})\cdot g(\mathbf{x},\boldsymbol{\Theta}_{1},\boldsymbol{\Theta}_{2}) $$ then $$ p(\boldsymbol{\Theta}_{2}\mid\mathbf{x},\boldsymbol{\Theta}_{1})\propto g(\mathbf{x},\boldsymbol{\Theta}_{1},\boldsymbol{\Theta}_{2}) $$ which follows since $ f(\mathbf{x}, \boldsymbol{\Theta}_{1}) $ is constant with respect to $ \boldsymbol{\Theta}_{2} $. With the prior given by Equation A2.19, we can express the resulting posterior distribution as $$ \begin{aligned}p(\mu\mid\mathbf{x})&\propto\ell(\mu\mid\mathbf{x})\cdot p(\mu)\\&\propto\exp\left[-\frac{(\mu-\mu_{0})^{2}}{2\sigma_{0}^{2}}-\frac{1}{2\sigma^{2}}\left(\sum_{i=1}^{n}x_{i}^{2}-2\mu n\overline{x}+n\mu^{2}\right)\right]\end{aligned} $$

We can factor out additional terms not involving $ \mu $ to obtain $$ p(\mu\mid\mathbf{x})\propto\exp\left(-\frac{\mu^{2}}{2\sigma_{0}^{2}}+\frac{\mu\mu_{0}}{\sigma_{0}^{2}}+\frac{\mu n\overline{x}}{\sigma^{2}}-\frac{n\mu^{2}}{2\sigma^{2}}\right) $$

Factoring in terms of $ \mu $, the term in the exponential becomes $$ -\frac{\mu^{2}}{2}\left(\frac{1}{\sigma_{0}^{2}}+\frac{n}{\sigma^{2}}\right)+\mu\left(\frac{\mu_{0}}{\sigma_{0}^{2}}+\frac{n\overline{x}}{\sigma^{2}}\right)=-\frac{\mu^{2}}{\sigma_{*}^{2}}+\frac{2\mu\mu_{*}}{2\sigma_{*}^{2}} $$ where $$ \sigma_{*}^{2}=\left(\frac{1}{\sigma_{0}^{2}}+\frac{n}{\sigma^{2}}\right)^{-1}\quad and\quad\mu_{*}=\sigma_{*}^{2}\left(\frac{\mu_{0}}{\sigma_{0}^{2}}+\frac{n\overline{x}}{\sigma^{2}}\right) $$

Finally, by completing the square, we have $$ p(\mu\mid\mathbf{x})\propto\exp\left[-\frac{(\mu-\mu_{*})^{2}}{2\sigma_{*}^{2}}+f(\mathbf{x},\mu_{0},\sigma^{2},\sigma_{0}^{2})\right] $$

Recalling Equation A2.20b, we can ignore the second term in the exponential (as it does not involve $ \mu $), and the resulting posterior for $ \mu $ (given the observed data x) becomes $$ p(\mu\mid\mathbf{x})\propto\exp\left[-\frac{(\mu-\mu_{*})^{2}}{2\sigma_{*}^{2}}\right] $$ demonstrating that the posterior density function for $ \mu $ is a normal with a mean of $ \mu_{*} $ and a variance of $ \sigma_{*}^{2} $, namely, $$ \mu\mid(\mathbf{x},\sigma^{2})\sim\mathrm{N}\left(\mu_{*},\sigma_{*}^{2}\right) $$
...
When expressed in terms of the scale ($ \lambda = 1/\beta $) parameter, the pdf becomes $$ p(x\mid\alpha,\lambda)=\frac{\lambda^{-\alpha}}{\Gamma(\alpha)}x^{\alpha-1}e^{-x/\lambda} $$ which yields $$ p(x\mid\alpha,\lambda)\propto x^{\alpha-1}e^{-x/\lambda} $$ Because both the rate and scale versions of the gamma distribution are widely used, take care to know which version your software package is using (for example, the default in R uses the scale parameter version). We can parameterize a gamma in terms of its mean and variance by noting that $$ \mu_{x}=\frac{\alpha}{\beta}=\alpha\lambda\qquad and\qquad\sigma_{x}^{2}=\frac{\alpha}{\beta^{2}}=\alpha\lambda^{2} $$ so that $$ \alpha=\frac{\mu_{x}^{2}}{\sigma_{x}^{2}}\qquad and\qquad\beta=\frac{\mu_{x}}{\sigma_{x}^{2}} $$

$ \Gamma(\alpha) $, the gamma function evaluated at $ \alpha $ (which normalizes the gamma distribution), is defined by $$ \Gamma(\alpha)=\int_{0}^{\infty}y^{\alpha-1}e^{-y}d y\qquad for\alpha>0 $$

This is the generalization of the factorial function from the integers to any positive number. If $n$ is an integer, then $\Gamma(n) = (n - 1)!$ Using integration by parts, one can show that $\Gamma$ satisfies the following identities $$ \Gamma(\alpha+1)=\alpha\Gamma(\alpha),\quad\Gamma(1)=1,\quad and\quad\Gamma(1/2)=\sqrt{\pi} $$ The chi-square ($ \chi^2 $) distribution is a special case of the gamma, as a $ \chi^2 $ random variable with $ n $ degrees of freedom follows a gamma distribution with parameters $ \alpha = n/2 $ and $ \beta = 1/2 $ ($ \lambda = 2 $), namely, $ \chi_n^2 \sim \Gamma(\frac{n}{2}, 1/2) $, giving the density function as $$ p(x\mid n)=\frac{2^{-n/2}}{\Gamma(n/2)}x^{n/2-1}e^{-x/2} $$ Hence for $ x \sim \chi_{n}^{2} $, $$ p(x)\propto x^{n/2-1}e^{-x/2} $$

The inverse-gamma distribution will prove useful as a conjugate prior for Gaussian likelihoods with unknown variance. It is defined by the distribution of the random variable $ y = x^{-1} $, where $ x \sim \text{Gamma}(\alpha, \beta) $. The resulting density function is $$ p(x\mid\alpha,\beta)=\frac{\beta^{\alpha}}{\Gamma(\alpha)}x^{-(\alpha+1)}e^{-\beta/x}\quad for\alpha,\beta,x>0 $$

The mean and variance for this distribution are only defined (i.e., finite) if $ \alpha $ is sufficiently large, with $$ \mu_{x}=\frac{\beta}{\alpha-1}\quad for\alpha>1\quad and\quad\sigma_{x}^{2}=\frac{\beta^{2}}{(\alpha-1)^{2}(\alpha-2)}\quad for\alpha>2 $$

Note for the inverse gamma that $$ p(x\mid\alpha,\beta)\propto x^{-(\alpha+1)}e^{-\beta/x} $$

If $ y \sim \chi_n^2 $, then $ x = 1/y $ follows an inverse chi-square distribution, which is denoted by $ x \sim \chi_n^{-2} $. This is a special case of the inverse gamma, with (as for a normal $ \chi^2 $) $ \alpha = n/2 $, $ \beta = 1/2 $. For $ n > 4 $ (i.e., $ \alpha > 2 $), the resulting density function is $$ p(x\mid n)=\frac{2^{-n/2}}{\Gamma(n/2)}x^{-(n/2+1)}e^{-1/(2x)} $$ with a mean and variance of $$ \mu_{x}=\frac{1}{n-2}\qquad and\qquad\sigma_{x}^{2}=\frac{2}{(n-2)^{2}(n-4)} $$

The scaled inverse chi-square distribution is more typically used in a Bayesian analysis, where the rate parameter, $ \beta $ (which equals 1/2 under a chi-square), is replaced by $ \beta = \sigma_{0}^{2}/2 $, making the resulting pdf $$ p(x\mid n)\propto x^{-(n/2+1)}e^{-\sigma_{0}^{2}/(2x)} $$ where the $ 1/(2x) $ term in the exponential is replaced by a $ \sigma_0^2/(2x) $ term. The scaled inverse chi-square distribution thus involves two parameters $ (\sigma_0^2 $ and $ n) $, and is denoted by $ \chi_{(n,\sigma_0^2)}^{-2} $ or $ \mathrm{SI}-\chi^2(n,\sigma_0^2) $. Note that if $$ x\sim\chi_{(n,\sigma_{0}^{2})}^{-2},\qquad\mathrm{t h e n}\qquad\sigma_{0}^{2}x\sim\chi_{n}^{-2} $$ which shows that $ \sigma_{0}^{2} $ is a scaling factor on a standard $ (\beta = 1/2) $ inverse chi-square.
...
We write the variance for the conditional mean prior in this way because $ \sigma^{2} $ is known (as we condition on it) and we scale $ \sigma^{2} $ by the hyperparameter, $ \kappa_{0} $. The resulting marginal posterior becomes $$ \sigma^{2}\left|\textbf{x}\sim\chi_{\nu_{n},\sigma_{n}^{2}}^{-2}\right.\quad\mathrm{a n d}\quad\mu\left|\textbf{x}\sim\textbf{t}_{\nu_{n}}\left(\mu_{n},\frac{\sigma_{n}^{2}}{\kappa_{n}}\right)\right. $$ where $ t_{n}(\mu,\sigma^{2}) $ denotes a t distribution with n degrees of freedom, mean $ \mu $, and scale parameter $ \sigma^{2} $, and where $$ \nu_{n}=\nu_{0}+n,\qquad\kappa_{n}=\kappa_{0}+n $$ $$ \mu_{n}=\mu_{0}\frac{\kappa_{0}}{\kappa_{n}}+\overline{x}\frac{n}{\kappa_{n}}=\mu_{0}\frac{\kappa_{0}}{\kappa_{0}+n}+\overline{x}\frac{n}{\kappa_{0}+n} $$ $$ \sigma_{n}^{2}=\frac{1}{\nu_{n}}\left(\nu_{0}\sigma_{0}^{2}+\sum_{i=1}^{n}\left(x_{i}-\overline{x}\right)^{2}+\frac{\kappa_{0}n}{\kappa_{n}}\left(\overline{x}-\mu_{0}\right)^{2}\right) $$

---

## appendix2_018 · Appendix: Introduction / CONJUGATE PRIORS
...
The Wishart is the sampling distribution for covariance matrices (just like the $ \chi^{2} $ is associated with the distribution of a sample variance for data drawn from a normal; Chapter 11). The pdf of the Wishart distribution is $$ p(\mathbf{W}\mid\mathbf{V})=2^{-n k/2}\pi^{-k(k-1)/k}\mid\mathbf{V}\mid^{-n/2}\mid\mathbf{W}\mid^{(n+k+1)/2}\frac{\exp\left(-\frac{1}{2}\mathbf{t}\mathbf{r}\left[\mathbf{V}^{-1}\mathbf{W}\right]\right)}{\prod_{i=1}^{k}\Gamma\left(\frac{n+1-i}{2}\right)} $$ Recall that the trace (tr) of a matrix is just the sum of its diagonal elements, $ \mathrm{tr}(\mathbf{A}) = \sum A_{ii} $ (Appendix 5). Odell and Feiveson (1966) presented an algorithm for generating random draws from the Wishart.

If $ \mathbf{Z} \sim \mathbf{W}_n(\mathbf{V}) $, then $ \mathbf{Z}^{-1} \sim \mathbf{W}_n^{-1}(\mathbf{V}^{-1}) $, where $ \mathbf{W}^{-1} $ denotes the inverse-Wishart distribution. The density function for an inverse-Wishart distributed random matrix, $ \mathbf{W} $, is $$ p(\mathbf{W}|\mathbf{V})=2^{-n k/2}\pi^{-k(k-1)/k}|\mathbf{V}|^{n/2}|\mathbf{W}|^{-(n+k+1)/2}\frac{\exp\left(-\frac{1}{2}\mathbf{t}\mathbf{r}\left[\mathbf{V}\mathbf{W}^{-1}\right]\right)}{\prod_{i=1}^{k}\Gamma\left(\frac{n+1-i}{2}\right)} $$ which is the distribution of the inverse of the sample covariance matrix.
