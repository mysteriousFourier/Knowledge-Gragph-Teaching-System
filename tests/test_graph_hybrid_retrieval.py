...
class GraphHybridRetrievalTest(unittest.TestCase):
    ...
    def test_explicit_graph_db_semantic_search_uses_text_mode(self):
    ...
    def test_default_retrieval_mode_is_hybrid(self):
        ...
        try:
            with patch.dict(
                os.environ,
                {"KGTS_VECTOR_INDEX_DIR": str(self.tmp_path / "vectors"), "KGTS_ALLOW_EXTERNAL_PATHS": "1"},
                clear=True,
            ):
                service = _service(self.tmp_path)
        finally:
        ...
        self.assertEqual(service.retrieval_mode, "hybrid")
        self.assertEqual(stats["mode"], "hybrid")
        self.assertEqual(stats["model"], "sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2")
    ...
    def test_hybrid_search_respects_allowed_node_scope(self):
        with patch.dict(os.environ, {"KGTS_RETRIEVAL_MODE": "hybrid", "KGTS_ALLOW_EXTERNAL_PATHS": "1"}):
            service = _service(self.tmp_path)
        service.vector_index = FakeVectorIndex()
        _seed_graph(service)

        results = service.semantic_search("matrix", top_k=5, allowed_node_ids=["matrix"])

        self.assertEqual([result["node_id"] for result in results], ["matrix"])

    def test_sparse_hybrid_search_uses_standard_library_scores(self):
        with patch.dict(os.environ, {"KGTS_RETRIEVAL_MODE": "sparse_hybrid", "KGTS_ALLOW_EXTERNAL_PATHS": "1"}):
            service = _service(self.tmp_path)
        _seed_graph(service)
    ...
    def test_rebuild_vector_index_is_not_limited_to_first_5000_nodes(self):
        with patch.dict(os.environ, {"KGTS_RETRIEVAL_MODE": "hybrid", "KGTS_ALLOW_EXTERNAL_PATHS": "1"}):
            service = _service(self.tmp_path)
        fake_index = FakeVectorIndex()
        service.vector_index = fake_index
        for index in range(5005):
            service.add_node(
                f"Content {index}",
                "concept",
                {"label": f"Node {index}", "id": f"node-{index:04d}"},
            )

        stats = service.rebuild_vector_index()

        self.assertEqual(fake_index.rebuild_calls, 1)
        self.assertEqual(stats["index_size"], 5005)
